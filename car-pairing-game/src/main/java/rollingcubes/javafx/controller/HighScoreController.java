...
public class HighScoreController {
    ...
    public void handleResetButton(ActionEvent actionEvent) throws IOException {
        ...
        ObservableList<GameResult> observableResult = FXCollections.observableArrayList();
        highScoreTable.setItems(observableResult);
        List<GameResult> highScoreList = gameResultDao.selectALL(gameResultDao.rowNumbers().size());
        log.info("Showing ALL rows of SCORE table!!!");
        observableResult.addAll(highScoreList);
        highScoreTable.setItems(observableResult);
        log.info("Show {} rows!!!", gameResultDao.rowNumbers().size());
    }

    public void handleHideButton(){
        ObservableList<GameResult> observableResult = FXCollections.observableArrayList();
        observableResult.removeAll();
